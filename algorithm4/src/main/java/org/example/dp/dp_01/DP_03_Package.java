...
public class DP_03_Package {
    ...
    public void test() {
        ...
        int[] w = {0, 3, 2, 1}; // 物品的重量
        int[] v = {0, 5, 2, 3}; // 物品的价值
        DP_03_Package exe = new DP_03_Package();
    ...
    int bag(int[] weights, int[] values, int canTake, int canWeight) {
        ...
        for (int n = 1; n <= canTake; n++) {
            for (int weight = 1; weight <= canWeight; weight++) {
                ...
                for (int k = 0; k <= weight / weights[n]; k++) {
                    dp[n][weight] = Math.max(dp[n][weight], dp[n - 1][weight - k * weights[n]] + k * values[n]);
                }
            }
        }
